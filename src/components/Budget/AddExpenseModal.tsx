...
export default function AddExpenseModal({
  ...
  const uploadReceipt = async (file: File, expenseId: string): Promise<string | null> => {
    try {
      ...
      const fileName = `${state.user?.id}/${expenseId}-${Date.now()}.${fileExt}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (receiptFile && expense) {
        receiptUrl = await uploadReceipt(receiptFile, expense.id);
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-auto max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-border">
          <h3 className="text-xl font-semibold text-foreground">Add New Expense</h3>
          <button
            onClick={onClose}
